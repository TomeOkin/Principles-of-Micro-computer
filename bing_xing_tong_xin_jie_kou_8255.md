...
若使8251A工作于内部同步，内部不复位，出现TE、OE、PE标志，复位许接收和发送，正常工作条件下，其命令指令字为11110111B。 
当8255A工作在方式1输出时，通知外设将数据取走的信号是OBF
8255A引脚信号WR=0,CS=0,A1=1,A0=1时，表示CPU向控制口送控制字
8255A在方式0工作时，端口A、B和C的输入输出可以有16种组合。
8255的A口工作在方式1输入时，其中断允许控制位INTE的开/关是通过对PC4的按位置位/复位操作完成的。
8255A的端口C按位置位复位控制字中的D0位决定对端口C的某一位置位或复位。
8255A的端口A的工作方式是由方式控制字的D6D5位决定。
8255A的端口B的工作方式由方式控制字的D2位决定。
